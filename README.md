...
| `query_code_graph` | Query the codebase knowledge graph using natural language. Ask questions like 'What functions call UserService.create_user?' or 'Show me all classes that implement the Repository interface'. Requires preflight: run list_projects -> select_active_project first. |
...
| `read_file` | Read the contents of a file from the project. Supports pagination for large files. Use this for implementation-level source verification, not as first step for relationship/hop analysis. |
