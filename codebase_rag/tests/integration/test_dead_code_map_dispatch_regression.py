...
class DispatchAwareAnalysisIngestor:
    ...
    def _count_inbound(self, qualified_name: str, rel_type: str) -> int:
        return sum(
            1
            for _, recorded_rel_type, to_spec, _ in self.relationships
            if recorded_rel_type == rel_type
            and to_spec[1] == cs.KEY_QUALIFIED_NAME
            and to_spec[2] == qualified_name
        )

    def _count_outbound(self, qualified_name: str, rel_type: str) -> int:
        return sum(
            1
...
def test_dead_code_pipeline_uses_dispatch_liveness_to_drop_handler_false_positives(
    temp_repo: Path,
) -> None:
    project = temp_repo / "dispatch_dead_code_regression"
    project.mkdir()
    (project / "dispatchers.py").write_text(
        (FIXTURE_DIR / "dynamic_dispatch.py").read_text(encoding="utf-8"),
        encoding="utf-8",
    )

    ingestor = DispatchAwareAnalysisIngestor()
    ...
    assert (
        "dispatch_dead_code_regression.dispatchers.handle_status"
        not in reported_symbols
    )
    assert (
        "dispatch_dead_code_regression.dispatchers.handle_reclaim"
        not in reported_symbols
    )
    assert dead_code_result["dead_code_except_test"]["filtered_dead_symbols"] >= 1
