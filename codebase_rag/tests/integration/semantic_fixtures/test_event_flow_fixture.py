...
from codebase_rag.tests.integration.semantic_fixtures.fixtures import EVENT_FLOW_FIXTURE
...
def test_event_flow_fixture_snapshot_is_deterministic(
    temp_repo: Path,
    mock_ingestor: MagicMock,
) -> None:
    fixture_repo = materialize_fixture_repo(temp_repo, EVENT_FLOW_FIXTURE)
    ...
    assert {
        str(cs.RelationshipType.WRITES_OUTBOX),
        str(cs.RelationshipType.PUBLISHES_EVENT),
        str(cs.RelationshipType.CONSUMES_EVENT),
        str(cs.RelationshipType.WRITES_DLQ),
        str(cs.RelationshipType.REPLAYS_EVENT),
        str(cs.RelationshipType.USES_HANDLER),
        str(cs.RelationshipType.USES_QUEUE),
    }.issubset(relationship_types)


@pytest.mark.anyio
async def test_event_flow_fixture_multi_hop_smoke(
    temp_repo: Path,
    ...
    result = await registry.multi_hop_analysis(
        qualified_name="event_flow_semantic_fixture.main.dispatch_invoice_created",
        depth=4,
        limit=20,
    )
    ...
    assert any(
        "InvoiceWorker.handle_invoice_created" in symbol for symbol in affected_symbols
    )
    ...
    assert relation_counts.get("PUBLISHES_EVENT", 0) >= 1
    assert relation_counts.get("USES_HANDLER", 0) >= 1
    assert relation_counts.get("WRITES_DLQ", 0) >= 1
